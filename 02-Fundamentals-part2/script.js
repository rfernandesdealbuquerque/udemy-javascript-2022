...
person.getSummary = function () {
    return (`${this.firstName} is a ${this.age}-year old ${this.job}, and he has ${this.hasDriversLicence ? 'a' : 'no'} driver's license`);
...
console.log(`${johnIsHigher ? john.fullName : mark.fullName}'s BMI (${johnIsHigher ? john.calcBMI() : mark.calcBMI()}) is higher than ${johnIsHigher ? mark.fullName : john.fullName}'s (${johnIsHigher ? mark.calcBMI() : john.calcBMI()})`);
